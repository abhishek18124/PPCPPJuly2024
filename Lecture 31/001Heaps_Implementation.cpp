...
class minHeap {
	...
	vector<int> v; // internal repr. of the minHeap
	...
	void heapify(int i) { // if you call heapify at index i, you've to make sure its left and right subtrees are already heaps
		...
		int minIdx = i; // assume the node at index i has the smallest value
		...
		if (minIdx != i) {
			swap(v[i], v[minIdx]); // heap prop. is now fixed at the ith index
			heapify(minIdx);
		}
