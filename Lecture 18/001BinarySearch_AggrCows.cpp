...
bool canPlaceCows(int c, int n, int pos[], int m) { // linear

	// check if you can place 'c' cows in 'n' stalls at a min-dist of >= m
...
int largestMinDist(int n, int c, int pos[]) {
	...
	while (s <= e) { // log(e-s)  * linear therefore time = n * log(e-s) where e-s is the size of the search space
		int m = s + (e - s) / 2;
